...
export async function POST( req: Request) {
    ...
    'SELECT admin_id, admin_password FROM admin WHERE admin_username = ? LIMIT 1',
    [username ]  
    );
      /*用户不存在 */
     if (rows.length === 0) {
      ...
      /*  比对密码 */
      const { admin_id, admin_password: pwdInDb } = rows[0];//等价于const admin_id   = rows[0].admin_id;   
      ...
      // 生成jwt
      const token = await encrypt(rows[0].admin_id.toString());
